...
HttpVersion_print(int fd, enum HttpVersion_t v) {
    ...
    switch (v) {
        ...
        case HTTP_TWO_ZERO:
            ...
            break;
        default:
            error("invalid http version", __FILE__, __LINE__);
            break;
    }
}
...
void ReqKind_print(int fd, enum ReqKind_t kind) {
    ...
    switch (kind) {
        ...
        default: {
            ...
            error("invalid method", __FILE__, __LINE__);
            break;
        }
    }
}

ReqLine_t ReqLine_new(enum ReqKind_t kind, char* uri, enum HttpVersion_t v) {
...
void RespKind_print(int fd, enum RespKind_t k) {
    ...
    switch (k) {
        ...
        case RESP_404:
            ...
            break;
        default:
            error("invalid response", __FILE__, __LINE__);
            break;
    }
}

RespLine_t RespLine_new(enum HttpVersion_t v, enum RespKind_t kind, char* info) {
...
////////////////////////////////////////
// http
Http_t Http_new(enum HttpKind_t kind, ReqLine_t reqLine, RespLine_t respLine, Header_t headers, char* body) {
