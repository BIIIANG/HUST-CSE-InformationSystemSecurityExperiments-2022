...
char* eatToken(enum TokenKind_t kind, int fd, int sepBySpace) {
    if (token.kind == kind) {
        char* s = token.lexeme;
...
ReqLine_t Parse_reqLine(int fd) {
    ...
    reqline = ReqLine_new(kind, uri, HTTP_ONE_ONE);
...
void setReqline(int kind, char* uri) {
    ReqLine_t t = ReqLine_new(kind, uri, HTTP_ONE_ONE);
    gReqline = t;
}

// parse whole request package, include reqLine, Headers, Body
void* Parse_parse(int fd, int reqOnly) {
    ...
    http = Http_new(HTTP_KIND_REQUEST, reqline, 0, 0, 0);
...
Http_t generate400() {
    ...
    respLine = RespLine_new(HTTP_ONE_ONE, RESP_400, "bad request");
    body = "";
    http = Http_new(kind, 0, respLine, header, body);
    return http;
}
